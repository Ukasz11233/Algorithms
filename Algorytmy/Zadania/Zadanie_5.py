...
def AvarageScore(A, n, lowest, highest):
    low = linearselect(A, lowest-1)
    high = linearselect(A, n-highest)
    result = []
    sum, i = 0, 0
    for el in A:
...
print(AvarageScore(A, len(A), 2, 2))

# O(n) - highest+lowest razy wywolanie QuickSelect, czyli (highest+lowest)*n + n + highest+lowest  = O(n)
